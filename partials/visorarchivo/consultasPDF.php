<?php
    ...
    $query=$Db->query("SELECT objetivo,alcance,definiciones,desarrollo,referencias,anexos 
    ...
    $consulta=$Db->query("SELECT nombre_registro,codigo,responsable,tiempo_resguardo 
                        FROM plantilla_completa_registros_obsoletos 
                        WHERE id_documento=$id_documento
                        AND consecutivo=$consec");
    $registros=array();
    $count=0;
    ...
    $consulta=$Db->query("SELECT fecha_cambio,descripcion_cambio,descripcion_justificacion,usuario
                    FROM plantilla_completa_revisiones_obsoletos 
                    WHERE id_documento=$id_documento
                    AND consecutivo=$consec
                    ");
    $revision=array();
